...
pub fn render_stat_tab(
    ...
    if let Some(err) = error {
        let block = Block::default()
            .title(format!(" My Jobs - Error: {} ", err))
            .borders(Borders::ALL)
    ...
    let rows: Vec<Row> = filtered
        ...
        .map(|m| {
            Row::new(vec![
                ...
                Cell::from(m.gpu_mem.as_str()),
                Cell::from(m.node.as_deref().unwrap_or("---")),
                Cell::from(m.gpu_type.as_deref().unwrap_or("---")),
            ])
        })
        .collect();

    let title = format!(" My Jobs ({} jobs) ", filtered.len());
    let table = Table::new(
        rows,
        [
